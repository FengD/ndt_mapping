...
class HdlGraphSlamMultiFrameNodelet : public nodelet::Nodelet {
public:
  typedef pcl::PointXYZI PointT;

  HdlGraphSlamMultiFrameNodelet() {
  }
  virtual ~HdlGraphSlamMultiFrameNodelet() {
  ...
  virtual void onInit() {
    ...
    dump_service_server = nh.advertiseService("/hdl_graph_slam/dump", &HdlGraphSlamMultiFrameNodelet::dump_service, this);
    save_map_service_server = nh.advertiseService("/hdl_graph_slam/save_map", &HdlGraphSlamMultiFrameNodelet::save_map_service, this);

    double graph_update_interval = private_nh.param<double>("graph_update_interval", 3.0);
    double map_cloud_update_interval = private_nh.param<double>("map_cloud_update_interval", 10.0);
    optimization_timer = mt_nh.createTimer(ros::Duration(graph_update_interval), &HdlGraphSlamMultiFrameNodelet::optimization_timer_callback, this);
    map_publish_timer = mt_nh.createWallTimer(ros::WallDuration(map_cloud_update_interval), &HdlGraphSlamMultiFrameNodelet::map_points_publish_timer_callback, this);
...
private:
  ...
  void cloud_callback(const sensor_msgs::PointCloud2ConstPtr& cloud_msg) {
    ...
    Eigen::Isometry3d odom = matrix2Isometry(pose);
  ...
  Eigen::Matrix4f matching(const ros::Time& stamp, const pcl::PointCloud<PointT>::ConstPtr& cloud) {
    ...
    if(delta_trans > keyframe_delta_trans || delta_angle > keyframe_delta_angle) {
      ...
      pcl::transformPointCloud(*keyframe, *tmp, odom);
      targetCloud += *tmp;
...
PLUGINLIB_EXPORT_CLASS(hdl_graph_slam::HdlGraphSlamMultiFrameNodelet, nodelet::Nodelet)
